...
### XBRLパターン修正の例
市場時価総額計算のように、XBRLデータの抽出パターンに問題がある場合：

1. **EDINETタクソノミーの確認**
   - 実際のXBRLデータから正しい要素名を確認
   - 存在しないパターン名を使用していないか検証
   - コンテキスト（CurrentYear, FilingDateInstant等）の確認

2. **パターン優先順位の設定**
   - `lib/edinet_common.py` のパターンリストで優先順位を管理
   - より具体的で正確なパターンを上位に配置
   - 汎用的なパターンは下位に配置

3. **動的検索の調整**
   - `lib/xbrl_parser.py` の動的検索優先度計算を更新
   - タグ名の部分一致による優先度付け
   - 数値範囲による妥当性チェック
