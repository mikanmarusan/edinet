---
name: running-tests
description: このリポジトリのテスト実行・追加・デバッグ手順。pytestスイートの実行、CIと同条件での再現、XBRLフィクスチャの追加、ゴールデン回帰ベースラインの再生成、Webビューアのnode:testスイート、実APIキーを使った手動スモークテストを扱う。「テストを実行」「テストを追加」「フィクスチャを追加」「ゴールデンベースラインを再生成」「CIが落ちる」と言われたときに使用する。
---
